...
while(!$overflow){
  ...
  while($moving==1 && $y < $floor){
#    print "moving grain $grains, from $x,$y; max y = $maxy\n";
    my $nexty = $y+1;
    if($map{"$x,$nexty"} != 1 && $nexty !=$floor){ #first try straight down
      $y++;
#      print "grain $grains moving straight down\n";
    }else{
      my $nextx=$x-1;
      if($map{"$nextx,$nexty"} != 1 && $nexty != $floor){ #try down-left
        $x--;$y++;
#        print "grain $grains moving down-left ($nextx,$nexty)\n";
      }else{
        $nextx=$x+1;
        if($map{"$nextx,$nexty"}!=1 && $nexty != $floor){ #try down-right
          $x++;$y++;
